...
console.log('\n[BACKSPIN — cue ball reverses past target]');
{
  ...
  // Red ball driven forward — under the two-regime physics (2026-06) the
  // rolling tail carries it all the way to the right cushion and back
  // past its start, so a final-x assertion is meaningless. Assert the
  // transfer happened via events instead: cue contacted red, and red
  // travelled far enough to reach the right rail.
...
// ──────────────────────────────────────────────────────────────────
// Pocket-mouth geometry — uses the REAL Side Pocket table (game.config
// values) because the mouth construction mirrors the rendered cushion
// segments. These pin the three behaviours from JJ's 2026-06 playtests:
// side pockets accept straight shots, balls never false-pot from
// outside the visible hole, and a ball can legitimately hang at the
// mouth edge on the felt.
// ──────────────────────────────────────────────────────────────────
...
console.log('\n[SP MOUTH — straight shot drops into the top-side pocket]');
{
  // Dead-centre shot at the top-side pocket. Under the pre-mouth model
  // this had a 1px capture window (pocket centre y=56 vs reflect line
  // y=97) and usually bounced off a cushion that visibly isn't there.
  const balls = [ball(0, 'white', 750, 300)];
  const r = simulateShot(
    balls,
    { power: 20, angle: -Math.PI / 2, spinX: 0, spinY: 0 }, // straight up
    spTable, spPhysics
  );
  const drops = r.events.filter(e => e.type === 'pocket_drop');
  assert(drops.length === 1, `ball dropped (got ${drops.length} drops)`);
  assert(drops.length === 1 && drops[0].pocketIdx === 1, `dropped into the top-side pocket (idx ${drops.length ? drops[0].pocketIdx : '-'})`);
}

console.log('\n[SP MOUTH — no false pot: ball resting near a pocket stays]');
{
  // Slow roller that dies ~75px from the top-side pocket centre — on
  // the felt, outside the visible 48px hole. The old radial model
  // (61px capture + 80px rest-rescue) teleport-potted this ball.
  const balls = [ball(0, 'white', 600, 130)];
  const r = simulateShot(
    balls,
    { power: 4.4, angle: 0, spinX: 0, spinY: 0 }, // gentle roll right
    spTable, spPhysics
  );
  const drops = r.events.filter(e => e.type === 'pocket_drop');
  const cueFinal = r.finalBalls.find(b => b.id === 0);
  assert(drops.length === 0, `no pocket_drop for a ball dying on the felt (got ${drops.length})`);
  assert(cueFinal.visible, 'ball still on the table');
  assert(cueFinal.position.y === 130, `ball path undisturbed (y=${cueFinal.position.y.toFixed(1)})`);
}

console.log('\n[SP MOUTH — ball can hang at the mouth edge on felt]');
{
  // Rolls toward the top-side pocket but dies just before the hole lip
  // (centre ~52px from pocket centre — outside the 48px visible hole,
  // inside the mouth span). Real tables and Miniclip both leave this
  // ball hanging on the felt at the jaws.
  const balls = [ball(0, 'white', 750, 300)];
  const r = simulateShot(
    balls,
    { power: 4.8, angle: -Math.PI / 2, spinX: 0, spinY: 0 },
    spTable, spPhysics
  );
  const drops = r.events.filter(e => e.type === 'pocket_drop');
  const cueFinal = r.finalBalls.find(b => b.id === 0);
  assert(drops.length === 0, `hanging ball not potted (got ${drops.length} drops)`);
  assert(cueFinal.visible, 'hanging ball still visible');
  assert(cueFinal.position.y > 97 && cueFinal.position.y < 130,
    `ball died at the mouth edge on felt (y=${cueFinal.position.y.toFixed(1)})`);
}
