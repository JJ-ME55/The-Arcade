...
// Module-level singleton — the most recently-created net client is
// exposed via getNetClient() so the multiplayer screens (which can't
// receive the instance via context easily during MP flow handshakes)
// have a way to reach it. setNetClient is called automatically when
// createCritterKartNet succeeds. Returns null when no client has been
// created yet (single-player mode).
let _currentNet: CritterKartNet | null = null;

export function getNetClient(): CritterKartNet | null {
    return _currentNet;
}
...
export async function createCritterKartNet(opts: NetOptions): Promise<CritterKartNet> {
    ...
    const client: CritterKartNet = {
        ...
        disconnect() {
            ...
            if (_currentNet === client) _currentNet = null;
        },
    };
    _currentNet = client;
    return client;
